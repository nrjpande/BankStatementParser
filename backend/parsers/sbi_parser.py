...
class SBIParser(BaseParser):
    ...
    def parse(self, df: pd.DataFrame) -> list:
        header_row = self.detect_header_row(df, ["value date", "description", "debit", "credit"])
        if header_row > 0:
        ...
        for col in df.columns:
            ...
            if "value" in cl and "date" in cl:
                col_map["date"] = col
            elif "txn" in cl and "date" in cl:
                col_map["date"] = col_map.get("date", col)
            elif "date" in cl and "date" not in col_map:
                col_map["date"] = col
            elif "description" in cl or "narration" in cl or "remarks" in cl:
