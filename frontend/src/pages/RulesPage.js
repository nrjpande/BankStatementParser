...
export default function RulesPage() {
  ...
  const COMMON_LEDGERS = [
    ...
    'Interest Received', 'Interest Paid', 'Sundry Debtors', 'Sundry Creditors',
    'GST Input', 'GST Output', 'Miscellaneous Expenses',
  ];
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <form onSubmit={handleCreate} className="flex items-end gap-4">
          ...
          <div className="flex-1">
            ...
            <select
              data-testid="rule-ledger-select"
              value={ledger}
              onChange={(e) => setLedger(e.target.value)}
              className="w-full h-10 px-4 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
              required
            >
        ...
        <p className="text-xs text-slate-400 mt-3">
          When a transaction's narration contains the keyword, the ledger will be automatically assigned.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {rules.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-slate-400 text-xs">Create rules above or map transactions in the table — rules are auto-created.</p>
          </div>
        ) : (
